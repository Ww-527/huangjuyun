...
def get_dataframe_from_excel():
    ...
    #sheet_name='销售数据'表示读取名为“销售表”的工作表的数据
    ...
    # df['时间']取出原有的“时间”这一列，其中包含交易的完整时间字符串，如10:25:30
...
def add_sidebar_func(df):
    ...
    with st.sidebar:
        # 添加侧边栏标题
        st.header('请筛选数据：')
        # 求数据框“城市”列去重复后的值，赋值给 city_unique
        city_unique = df['城市'].unique()
        city = st.multiselect(
            '请选择城市：',
            options=city_unique, #将所有选项设置为 city_unique
            default=city_unique,  # 第1次的默认选项为 city_unique
        )
        
        # 求数据库“顾客类型”列去重复后的值，赋值给 customer_type_unique
        customer_type_unique = df['顾客类型'].unique()
        customer_type = st.multiselect(
            '请选择顾客类型：',
            options=customer_type_unique, #将所有选项设置为 customer_type_unique
            default=customer_type_unique,  # 第1次的默认选项为 customer_type_unique
        )
        
        # 求数据库“性别”列去重复后的值，赋值给 gender_unique 
        gender_unique = df['性别'].unique()
        gender = st.multiselect(
            '请选择性别',
            options=gender_unique,  #将所有选项设置为 gender_unique
            default=gender_unique,  # 第1次的默认选项为 gender_unique
        )
...
def main_page_demo(df):
    ...
    # 选中数据框中的'评分'列，使用mean()计算'评分'列的平均值，使用round()四舍五入保留一位小数
    #保留一位小数
    ...
    # 选中数据框中的'总价'列，使用mean()计算'评分'列的平均值，使用round()四舍五入保留一位小数
    #保留俩位小数
